...
class LikesSpider:
    ...
    def __init__(self, collection_param, table_param):
        ...
        self.node = None
        self.count = 1
        # self.mongo_client = pymongo.MongoClient(LOCAL_MONGO_HOST, LOCAL_MONGO_PORT)
        # self.collection = self.mongo_client[DB_NAME]["Likes"]
        # self.table = self.mongo_client[DB_NAME]["FollowedIDs"]
        self.collection = collection_param
        self.table = table_param

        os.system('pkill -f phantom')
        self.browser = webdriver.PhantomJS(executable_path=r'C:\phantomjs-2.1.1-windows\bin\phantomjs.exe')
    ...
    #  获取每一条微博的信息
    def tweet_spider(self, uid, tweets):
...
if __name__ == "__main__":
    ...
    # LikesSpider(collection, table).page_nums_spider("1290682531")
